...
export function SourceControlChangesView({ gitService: propGitService, projectPath }: SourceControlChangesViewProps) {
  ...
  const commitDisabledReason = !staged.length && !commitMessage.trim() ? sectionTitle("先选择文件并填写版本说明", "Select files and add a commit message") : !staged.length ? sectionTitle("请先选择要保存的文件", "Select files to save first") : !commitMessage.trim() ? sectionTitle("请填写版本说明", "Add a commit message") : null
  ...
  return <VStack spacing={0} alignment="leading" frame={{ maxWidth: "infinity", maxHeight: "infinity", alignment: "top" }}>
    ...
    <Section header={<Text>{sectionTitle("本次版本", "Current Version")}</Text>}>
      <VStack spacing={8} alignment="leading">
        ...
        <Text font="caption" foregroundStyle="secondaryLabel">{commitDisabledReason || syncSummary}</Text>
