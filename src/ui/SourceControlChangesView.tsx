...
function HistoryPreview({ commit, syncState, onSelect }: { commit: GitCommitInfo; syncState: "synced" | "local" | "unknown"; onSelect: () => void }) { return <VStack spacing={2} alignment="leading" onTapGesture={onSelect} frame={{ maxWidth: "infinity", minHeight: 58, alignment: "leading" }}><Text font="headline" lineLimit={2}>{commit.message}</Text><Text font="caption" foregroundStyle="secondaryLabel">{formatHistoryTime(commit.timestamp)} · {commit.shortOid}</Text><Text font="caption" foregroundStyle={syncState === "local" ? "orange" : "secondaryLabel"}>{syncState === "synced" ? "✓ 已同步" : syncState === "local" ? "仅本地" : "同步状态未知"}</Text></VStack> }
