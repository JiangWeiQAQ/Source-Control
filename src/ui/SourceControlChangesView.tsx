...
function EmptyChangesView({ loading, t, language, showCommittedFiles, onToggleCommittedFiles }: { loading: boolean; t: (key: string) => string; language: AppLanguage; showCommittedFiles: boolean; onToggleCommittedFiles: () => void }) {
  ...
  return <>
    <Section>
      <VStack spacing={8} alignment="center" frame={{ maxWidth: "infinity", minHeight: 144, alignment: "center" }} padding={{ top: 20, bottom: 20 }}>
        ...
        <Text font="headline" frame={{ maxWidth: "infinity", alignment: "center" }}>{loading ? t("fetching") : isChinese ? "工作区干净" : "Working Tree Clean"}</Text>
        <Text font="footnote" foregroundStyle="secondaryLabel" frame={{ maxWidth: "infinity", alignment: "center" }}>{loading ? t("fetching") : isChinese ? "所有本地改动均已保存。" : "All local changes have been saved."}</Text>
      </VStack>
    </Section>
    <Section>
      <Button action={onToggleCommittedFiles} buttonStyle="plain" contentShape={{ kind: "interaction", shape: "rect" }}>
        <HStack spacing={8} alignment="center" frame={{ maxWidth: "infinity", minHeight: 52, alignment: "leading" }} padding={{ horizontal: 14, vertical: 10 }} background="secondarySystemBackground" clipShape={{ type: "rect", cornerRadius: 12 }}>
          <Image systemName="checkmark.circle" foregroundStyle="secondaryLabel" />
          <Text font="subheadline">{isChinese ? "已提交文件" : "Committed Files"}</Text>
          <Spacer />
          <Image systemName={showCommittedFiles ? "chevron.up" : "chevron.right"} foregroundStyle="secondaryLabel" />
        </HStack>
      </Button>
      {showCommittedFiles ? <VStack spacing={4} alignment="center" frame={{ maxWidth: "infinity", alignment: "center" }} padding={{ horizontal: 14, vertical: 14 }} background="secondarySystemBackground" clipShape={{ type: "rect", cornerRadius: 12 }}><Text font="footnote" foregroundStyle="secondaryLabel" frame={{ maxWidth: "infinity", alignment: "center" }}>{isChinese ? "当前版本文件列表暂不可用。" : "The current version file list is unavailable."}</Text></VStack> : null}
    </Section>
  </>
}
...
export function SourceControlChangesView({ gitService: propGitService, projectPath }: SourceControlChangesViewProps) {
  ...
  return <List navigationTitle="Changes" toolbar={{ topBarLeading: <Button title={t("close")} systemImage="xmark" buttonStyle="borderless" action={() => dismiss()} />, topBarTrailing: <HStack spacing={8}><Button title={t("settings")} systemImage="gearshape" buttonStyle="borderless" action={async () => { await Navigation.present(<SourceControlSettingsView gitService={service} />); await loadStatus() }} /><Button title={t("refresh")} systemImage="arrow.clockwise" buttonStyle="borderless" disabled={loading} action={loadStatus} /></HStack> }}>
      ...
      <Button action={async () => { try { await Navigation.present(<SourceControlHistoryCompareView gitService={service} language={language} />) } catch (error) { const message = error instanceof Error ? error.message : String(error); console.error("[Changes] failed to open version compare", error); setErrorMessage(`${sectionTitle("无法打开版本对照", "Unable to open version compare")}\n${message}`) } }} buttonStyle="plain" contentShape={{ kind: "interaction", shape: "rect" }}>
        <HStack spacing={8} alignment="center" frame={{ maxWidth: "infinity", minHeight: 58, alignment: "leading" }} padding={{ horizontal: 14, vertical: 10 }} background="secondarySystemBackground" clipShape={{ type: "rect", cornerRadius: 12 }}>
          ...
          <VStack spacing={2} alignment="leading">
            ...
            <Text font="caption" foregroundStyle="secondaryLabel">{sectionTitle("查看本地与 GitHub 提交记录", "Compare local and GitHub commits")}</Text>
          </VStack>
          <Spacer />
    ...
    <Section header={<Text>{sectionTitle("GitHub 同步", "GitHub Sync")}</Text>}>
      <Button action={async () => { try { await Navigation.present(<SourceControlRemoteView gitService={service} onChanged={loadStatus} />); await loadStatus() } catch (error) { const message = error instanceof Error ? error.message : String(error); console.error("[Changes] failed to open GitHub sync", error); setErrorMessage(`${sectionTitle("无法打开 GitHub 同步", "Unable to open GitHub Sync")}\n${message}`) } }} buttonStyle="plain" contentShape={{ kind: "interaction", shape: "rect" }}>
        <HStack spacing={8} alignment="center" frame={{ maxWidth: "infinity", minHeight: 58, alignment: "leading" }} padding={{ horizontal: 14, vertical: 10 }} background="secondarySystemBackground" clipShape={{ type: "rect", cornerRadius: 12 }}>
          <Image systemName="arrow.triangle.2.circlepath" foregroundStyle={syncDiverged || syncBehind > 0 ? "orange" : syncAhead > 0 ? "blue" : "green"} />
          <VStack spacing={2} alignment="leading"><Text font="subheadline">{hasRemote ? syncText : sectionTitle("尚未连接 GitHub", "Not connected to GitHub")}</Text><Text font="caption" foregroundStyle="secondaryLabel">{hasRemote ? sectionTitle("管理同步设置", "Manage Sync Settings") : sectionTitle("设置 GitHub 同步", "Set Up GitHub Sync")}</Text></VStack>
          <Spacer />
          <Image systemName="chevron.right" foregroundStyle="secondaryLabel" />
        </HStack>
      </Button>
    </Section>
    {status?.isClean ? <EmptyChangesView loading={loading} t={(key) => t(key as never)} language={language} showCommittedFiles={showCommittedFiles} onToggleCommittedFiles={() => setShowCommittedFiles((value) => !value)} /> : null}
