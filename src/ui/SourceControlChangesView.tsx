...
export function SourceControlChangesView({ gitService: propGitService, projectPath }: SourceControlChangesViewProps) {
  ...
  const loadAllFiles = async () => { if (!projectPath) { setAllFiles([]); return }; try { setAllFiles(await enumerateProjectFiles(projectPath)) } catch { console.error("[AllFiles] read failed"); setAllFiles([]) } }
